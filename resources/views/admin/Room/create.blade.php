...
<style>
  ...
  .cap-wrap {
    ...
    border-radius:10px; padding:12px 16px; margin-top:.75rem; display:none;
  }
  ...
  /* gate overlay */
  .grid-gate {
    text-align:center; padding:3rem; color:var(--muted); font-size:13px;
    border:1px dashed var(--border); border-radius:10px;
    display:flex; flex-direction:column; align-items:center; gap:.75rem;
  }
  .grid-gate-icon { font-size:1.8rem; opacity:.3; }
  ...
  /* scope modifier: đơn lẻ / hàng / cột */
  .scope-btn {
  ...
  /* highlight hàng / cột khi hover trong bulk mode */
  ...
  /* row-label clickable khi ở mode row */
  ...
  /* col-header row (số cột, clickable trong col mode) */
  ...
  .col-hdr-spacer { width:20px; flex-shrink:0; } /* lề bên trái (= row-lbl width) */
  .col-hdr-aisle { width:18px; flex-shrink:0; } /* lối đi placeholder */
  ...
  .btn-gen:disabled { opacity:.35; cursor:not-allowed; border-color:var(--border); color:var(--muted); }
  ...
  <div class="container-fluid px-3 px-md-4">
    ...
    <form method="POST" action="{{ route('admin.rooms.store') }}" id="createRoomForm">
        ...
        <div class="row g-3">
          <div class="col-md-6">
            ...
            <input type="text" class="cw-input" id="room_name" name="room_name"
              placeholder="VD: Phòng 1, Cinema Gold…" value="{{ old('room_name') }}" required>
      ...
      <div class="cw-card">
        ...
        <div class="grid-ctrl">
          <div>
            ...
            <input type="number" class="cw-input" id="inp_rows" name="rows" min="1" max="26" value="{{ old('rows', 6) }}" disabled>
          </div>
          <div>
            <label class="cw-label" for="inp_cols">Số cột mỗi hàng</label>
            <input type="number" class="cw-input" id="inp_cols" name="columns" min="1" max="30" value="{{ old('columns', 10) }}" disabled>
          </div>
          <div>
            <label class="cw-label" for="inp_def">Loại ghế mặc định</label>
            <div class="cw-select-wrap">
        ...
        <p class="hint" id="hintText">← Chọn loại phòng chiếu trước để mở khóa thiết kế sơ đồ ghế</p>
      </div>
      ...
      <div class="cw-card">
        ...
        <div class="screen-area" id="screenArea" style="display:none">
          <div class="screen-bar"></div>
          <div class="screen-lbl">MÀN HÌNH</div>
        </div>

        <div class="seat-grid-wrap">
          <div class="seat-grid-center" id="seatGridCenter">
            <div class="grid-gate" id="gridGate">
              <div class="grid-gate-icon">🎬</div>
              <span>Chọn loại phòng và nhấn <strong>Tạo sơ đồ</strong> để bắt đầu</span>
            </div>
          </div>
        </div>
      ...
      {{-- 04 THỐNG KÊ --}}
      <div class="cw-card" id="statsCard" style="display:none">
...
{{-- old() values cho restore sau validation fail --}}
<div id="formMeta"
  data-old-type="{{ old('type_id') }}"
  data-old-rows="{{ old('rows') }}"
  data-old-cols="{{ old('columns') }}"
...
/* ── State ── */
let activeTool = SEAT_TYPES[0]?.id ?? null; // seat type id | 'aisle' | 'erase'
let activeScope = 'single';                  // 'single' | 'row' | 'col'
let gridData = [];   // gridData[r][c] = { type_id } | null
let aisles   = [];   // cột lối đi (0-based index)
let nRows = 0, nCols = 0, maxCap = 0;

/* ════════════════════════════════
   PALETTE — loại ghế + công cụ
════════════════════════════════ */
function buildPalette() {
  ...
  aisleBtn.title = 'Click vào cột bất kỳ để chèn lối đi';
...
function setScope(scope) {
  // "Lối đi" luôn dùng scope col — không cho đổi
  if (activeTool === 'aisle') return;
  activeScope = scope;
  updateScopeBtns();
  renderGrid(); // re-render để cập nhật cursor
}
...
function handleRoomTypeChange() {
  ...
  const hasType = maxCap > 0;
  document.getElementById('capWrap').classList.toggle('show', hasType);
  refreshCapBar();
  ['inp_rows','inp_cols','inp_def'].forEach(id => document.getElementById(id).disabled = !hasType);
  document.getElementById('btnGenerate').disabled = !hasType;
  document.getElementById('hintText').textContent = hasType
    ? 'Tạo sơ đồ → chọn loại ghế + phạm vi → click để áp dụng. Click nhãn hàng (A B C…) để đổi cả hàng, click số cột để đổi cả cột.'
    : '← Chọn loại phòng chiếu trước để mở khóa thiết kế sơ đồ ghế';
}
...
function generateGrid() {
  if (!maxCap) { showToast('Vui lòng chọn loại phòng chiếu trước!'); return; }
  nRows = Math.min(26, Math.max(1, parseInt(document.getElementById('inp_rows').value) || 6));
  nCols = Math.min(30, Math.max(1, parseInt(document.getElementById('inp_cols').value) || 10));
  const defId = parseInt(document.getElementById('inp_def').value) || SEAT_TYPES[0]?.id;

  if (nRows * nCols > maxCap) {
    showToast(`${nRows} × ${nCols} = ${nRows*nCols} ghế — vượt sức chứa ${maxCap}. Hãy giảm số hàng / cột.`);
    return;
  }
  ...
  document.getElementById('gridGate').style.display = 'none';
}
...
function renderGrid() {
  ...
  // ── Col header row (số thứ tự cột, clickable khi scope=col) ──
  ...
  for (let c = 0; c < nCols; c++) {
    if (aisles.includes(c)) {
      const aisleHdr = document.createElement('div');
      aisleHdr.className = 'col-hdr-aisle';
      colHdrRow.appendChild(aisleHdr);
    }
  ...
  for (let r = 0; r < nRows; r++) {
    ...
    // Row label (A B C…) — clickable khi scope=row
...
function applyRow(r) {
  if (activeTool === 'erase') {
    ...
    // kiểm tra capacity: đếm ghế rỗng trong hàng sẽ bị fill
    const emptyInRow = gridData[r].filter(c => c === null).length;
    if (countSeats() + emptyInRow > maxCap) {
      showToast(`Thêm cả hàng ${String.fromCharCode(65+r)} sẽ vượt sức chứa ${maxCap} ghế!`);
      return;
    }
...
function applyCol(c) {
  if (activeTool === 'erase') {
    ...
    if (countSeats() + emptyInCol > maxCap) {
      showToast(`Thêm cả cột ${c+1} sẽ vượt sức chứa ${maxCap} ghế!`);
      return;
    }
...
function handleSeatClick(r, c, el) {
  ...
  if (activeTool === 'erase') {
    ...
    if (wasEmpty && countSeats() >= maxCap) {
      showToast(`Đã đạt sức chứa tối đa ${maxCap} ghế!`);
      return;
    }
...
document.getElementById('createRoomForm').addEventListener('submit', function(e) {
  ...
  if (!seats.length) {
    e.preventDefault(); showToast('Vui lòng tạo sơ đồ ghế trước khi lưu.'); return;
  }
...
(function () {
  ...
  if (!meta) return;
  const oldType = meta.dataset.oldType;
  const oldRows = meta.dataset.oldRows;
  const oldCols = meta.dataset.oldCols;
  if (!oldType) return;
  document.addEventListener('DOMContentLoaded', () => {
    document.getElementById('type_id').dispatchEvent(new Event('change'));
    if (oldRows && oldCols) generateGrid();
  });
})();
</script>
