...
<div class="cw">
  <div class="container-fluid px-4">
    <div class="cw-head"><h2>Chỉnh sửa phòng chiếu & Thiết kế sơ đồ</h2></div>
      ...
      <div class="cw-card">
        ...
        <div class="row g-3">
          <div class="col-md-6">
            ...
            <input type="text" class="cw-input" name="room_name" value="{{ old('room_name', $room->room_name) }}" placeholder="VD: Phòng Gold 01" required>
          </div>
          <div class="col-md-6">
            ...
            <div class="cw-select-wrap">
              <select class="cw-select" id="type_id" name="type_id" required onchange="handleRoomTypeChange()">
                ...
                @foreach($roomTypes as $rt)
                  <option value="{{ $rt->id }}" data-capacity="{{ $rt->capacity }}" {{ old('type_id', $room->type_id) == $rt->id ? 'selected' : '' }}>{{ $rt->type }} (Max: {{ $rt->capacity }} ghế)</option>
                @endforeach
              </select>
            </div>
          </div>
      ...
      <div class="cw-card">
        ...
        <div class="grid-ctrl">
          <div>
            ...
            <input type="number" class="cw-input" id="inp_rows" min="1" max="26" value="{{ $maxRow }}">
          </div>
          <div>
            <label class="cw-label">Số cột (Bắt buộc số chẵn)</label>
            <input type="number" class="cw-input" id="inp_cols" min="1" max="30" value="{{ $maxCol }}">
          </div>
          <div>
          ...
          <button type="button" class="btn-gen" id="btnGenerate" onclick="generateGrid()">↺ Tạo sơ đồ</button>
        </div>
      </div>

      {{-- 03 SƠ ĐỒ --}}
      <div class="cw-card" id="designArea">
...
[
  ...
  { "row": {{ $seat->row }}, "column": {{ $seat->column }}, "type_id": {{ $seat->type_id ?? 'null' }} }{{ !$loop->last ? ',' : '' }}
  @endforeach
]
</script>
...
function loadExistingSeats() {
    ...
    // 1. Khởi tạo grid với giá trị null
    gridData = [];
    for (let r = 0; r < nRows; r++) {
        gridData[r] = [];
        for (let c = 0; c < nCols; c++) gridData[r][c] = { type_id: null };
    }
    
    // 2. Đặt ghế hiện có vào đúng vị trí
    EXISTING.forEach(s => {
        const r = s.row - 1;
        const c = s.column - 1;
    ...
    // 3. QUAN TRỌNG: Nếu hàng cuối là hàng Couple, loại bỏ các ô null thừa 
    // để justify-content: center có thể hoạt động chính xác
    if (nRows > 0 && isRowCouple(nRows - 1)) {
        gridData[nRows - 1] = gridData[nRows - 1].filter(cell => cell.type_id !== null);
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    // Set maxCap from selected room type
    const sel = document.getElementById('type_id');
    maxCap = parseInt(sel.options[sel.selectedIndex].dataset.capacity) || 0;
    document.getElementById('capWrap').classList.toggle('show', maxCap > 0);
    ...
    // Gọi handleRoomTypeChange để enable các nút (nếu đã chọn room type)
    if (maxCap > 0) {
