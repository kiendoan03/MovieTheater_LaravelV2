...
<style>
  ...
  .movie-preview-poster {
    width:44px; height:64px; border-radius:6px; object-fit:cover;
    background:rgba(255,255,255,.05); flex-shrink:0;
  }
  .movie-preview-info { flex:1; min-width:0; }
  ...
  @media(max-width:768px) { .time-row { grid-template-columns:1fr 1fr; } }
  ...
  .schedule-item-gap {
    font-size:11px; padding:2px 8px; border-radius:20px; flex-shrink:0;
  }
  ...
  .room-info {
    background:var(--surface); border:1px solid var(--border); border-radius:10px;
    padding:12px 16px; margin-top:.75rem; display:none; flex-wrap:wrap; gap:16px;
  }
  .room-info.show { display:flex; }
    ...
    <form method="POST" action="{{ route('admin.schedules.store') }}" id="scheduleForm">
          ...
          <select class="sw-select" id="movie_id" name="movie_id" required onchange="onMovieChange()">
            ...
            @foreach($movies as $m)
              <option value="{{ $m->id }}"
                ...
                data-genre="{{ $m->categories->first()?->name ?? '' }}"
                data-title="{{ $m->movie_name ?? '' }}"
                {{ old('movie_id') == $m->id ? 'selected' : '' }}>
      ...
      <div class="sw-card">
        ...
        <div class="room-info" id="roomInfo">
          <div class="room-info-item"><strong id="roomInfoName">—</strong>Tên phòng</div>
          <div class="room-info-item"><strong id="roomInfoType">—</strong>Loại phòng</div>
          <div class="room-info-item"><strong id="roomInfoCapacity">—</strong>Sức chứa</div>
        </div>
      </div>
      ...
      <div class="sw-card">
        ...
        <div class="row g-3 mb-3">
          <div class="col-md-6">
            ...
            <input type="datetime-local" class="sw-input" id="start_time" name="start_time"
              value="{{ old('start_time', \Carbon\Carbon::now('Asia/Ho_Chi_Minh')->format('Y-m-d\TH:i')) }}"
        ...
        {{-- Danh sách lịch chiếu cùng phòng cùng ngày --}}
...
function onMovieChange() {
  ...
  const dur = parseInt(opt.dataset.duration) || null;
  selectedMovieDuration = dur;
  ...
  if (opt.value) {
    ...
    document.getElementById('previewDuration').textContent = dur ? `⏱ ${dur} phút` : 'Thời lượng chưa cập nhật';
  } else {
...
/* ── Load existing schedules via fetch ── */
async function loadExistingSchedules() {
  ...
  try {
    const res  = await fetch(`/Admin/Schedule/by-room?room_id=${roomId}&date=${date}`);
...
document.addEventListener('DOMContentLoaded', () => {
  if (document.getElementById('movie_id').value) onMovieChange();
  if (document.getElementById('room_id').value)  onRoomChange();
});
</script>
